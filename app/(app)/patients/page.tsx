  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch('patients', {
        method: 'POST',
        body: JSON.stringify({ name, blood_group: bloodGroup, condition_info: conditionInfo }),
      });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage blood recipient patients in the system.</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-slate-900 p-6 rounded-lg border border-slate-800">
          <h2 className="text-lg font-medium text-white mb-4">Register New Patient</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">Blood Group Needed</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-slate-400 mb-1">Condition Info / Notes</label>
              <input required type="text" value={conditionInfo} onChange={e => setConditionInfo(e.target.value)} className="w-full px-3 py-2 border border-slate-700 bg-slate-800 text-white rounded-md focus:ring-red-500 focus:border-red-500" />
            </div>
            <div className="md:col-span-2 mt-2">
              <button type="submit" className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition font-medium">Save Patient</button>
            </div>
          </form>
        </div>
      )}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4 bg-slate-950">Condition</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} className="py-4 text-center text-sm text-slate-500 border-b border-slate-800">No patients found.</td></tr>
              ) : (
                filteredPatients.map((patient) => (
                  <tr key={patient.id}>
                    <td className="py-2 px-4 text-sm border-b border-slate-800 text-slate-400">{patient.id}</td>
                    <td className="py-2 px-4 text-sm border-b border-slate-800 text-slate-200">{patient.name}</td>
                    <td className="py-2 px-4 text-sm border-b border-slate-800">
                      <span className="bg-red-500/10 text-red-500 px-2 rounded text-[10px] font-bold">{patient.blood_group}</span>
                    </td>
                    <td className="py-2 px-4 text-sm border-b border-slate-800 text-slate-400">{patient.condition_info}</td>
                  </tr>
                ))
              )}
            </tbody>
